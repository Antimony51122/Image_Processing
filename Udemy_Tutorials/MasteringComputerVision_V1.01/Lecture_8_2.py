...
image = cv2.imread('images/digits.png')  # raw data set
...
# use numpy to split into this form
...
# Use numpy reshape function to flatten that 20x20 array into a row
...
# ret, result, neighbors, distance = knn.find_nearest(test, k=3)  # these 3 lines out of date
knn = cv2.ml.KNearest_create()
knn.train(train, cv2.ml.ROW_SAMPLE, train_labels)
ret, result, neighbours, distance = knn.findNearest(test, k=3)
# tested with k=5 and 6, the accuracy went down

# Now we check the accuracy of classification
# For that, compare the result with test_labels and check which are wrong
matches = result == test_labels  # match = how many times result matches the label
correct = np.count_nonzero(matches)  # count how many matches true
accuracy = correct * (100.0 / result.size)  # size giving the number of total number in results itself
...
def makeSquare(not_square):
    ...
    if (height == width):
        ...
        if (height > width):
            pad = (height - width) / 2
            # print("Padding = ", pad)
            doublesize_square = cv2.copyMakeBorder(doublesize, 0, 0, cv2.BORDER_CONSTANT, value=BLACK)
        else:
            pad = (width - height) / 2
            # print("Padding = ", pad)
...
for c in contours:
    ...
    if w >= 5 and h >= 25:  # adds a filter
        roi = blurred[y: y + h, x: x + w]
        ret, roi = cv2.threshold(roi, 127, 255, cv2.THRESH_BINARY_INV)
        squared = makeSquare(roi)
        final = resize_to_pixel(20, squared)
        cv2.imshow("final", final)
        final_array = final.reshape((1, 400))  # 20x20 to 1x400
